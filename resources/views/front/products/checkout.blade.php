          ...
          <div class="col-lg-8 col-md-8 col-sm-12 col-xs-12 ">
            <div class="ps-checkout__billing">
              ...
              <div class="form-group form-group--inline">
                ...
                <input name="billing_firstname" value="{{old('billing_firstname',$user->name)}}" class="form-control" type="text">
              </div>
              <div class="form-group form-group--inline">
                <label>Last Name<span>*</span>
                </label>
                <input name="billing_lastname" value="{{old('billing_lastname',$user->name)}}" class="form-control" type="text">
              </div>
              <div class="form-group form-group--inline">
                <label>Email Address<span>*</span>
                </label>
                <input name="billing_email" value="{{old('billing_email',$user->email)}}" class="form-control" type="email">
              </div>
              <div class="form-group form-group--inline">
                <label>Phone<span>*</span>
                </label>
                <input name="billing_phone" value="{{old('billing_phone',$user->mobile)}}" class="form-control" type="text">
              </div>
              <div class="form-group form-group--inline">
                <label>Address<span>*</span>
                </label>
                <input name="billing_address" value="{{old('billing_address')}}" class="form-control" type="text">
              </div>
              <div class="form-group form-group--inline">
                <label>City<span>*</span>
                </label>
                <input name="billing_city" value="{{old('billing_city')}}" class="form-control" type="text">
              </div>
              <div class="form-group form-group--inline">
                <label>Postal Code<span>*</span>
                </label>
                <input name="billing_postalcode" value="{{old('billing_postalcode')}}" class="form-control" type="text">
              </div>
              <div class="form-group form-group--inline">
                ...
                <table class="table ps-checkout__products">
                  ...
                  <tbody>
                    ...
                    @foreach($cart as $item)
                       ...
                       @endphp 
                    <tr>
                      <td>@php  $i++;  @endphp {{$i}}-  {{$item->product->name}} x {{$item->quantity}}</td>
                      <td>{{ $item->quantity * $item->product->purchase_price }}</td>
                    </tr>
                    @endforeach
                    <tr> 
                      <!-- Shopping Cart - Part 4
دقيقة 55 فيه كود في الريبوستروي -->
                      <td>ORDER TOTAL</td>
                      <td>{{ $total }} $</td>
                    </tr>
                  </tbody>
